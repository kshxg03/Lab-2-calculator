...
from app.schemas import BaseExpression, Expression, CalculatorLog


def test_expression_and_calculator_log_models():
    """Test schema instantiation and inheritance."""
    req = Expression(expr="10 + 20")
    assert req.expr == "10 + 20"
    assert isinstance(req, BaseExpression)

    log = CalculatorLog(expr="10 + 20", result=30, timestamp="2026-01-01T00:00:00")
    assert log.expr == "10 + 20"
    assert log.result == 30
    assert isinstance(log, BaseExpression)
